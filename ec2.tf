...
resource "aws_key_pair" "new" {
  key_name   = "myKey" # The name of the key pair on AWS
...
resource "aws_launch_template" "ec2_lt" {
  name = "${local.env}-ec2-lt-${local.region}"
...
git clone https://github.com/aswath-pt/node-js-sample.git
cd node-js-sample
sudo npm install
...
resource "aws_autoscaling_group" "ec2_asg" {
  name = "${local.env}-ec2-asg-${local.region}"
  min_size = 1
  max_size = 1
  desired_capacity = 1
...
resource "aws_security_group" "alb_sg" {
  name = "${local.env}-alb-sg-${local.region}"
...
resource "aws_lb_target_group" "ec2_tg" {
  name = "${local.env}-ec2-tg-${local.region}"
  port = 5000
  protocol = "HTTP"
  vpc_id = aws_vpc.vpc.id 
